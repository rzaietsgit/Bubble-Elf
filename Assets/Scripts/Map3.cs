...
public class Map3 : MonoBehaviour
{
	...
	public void UpdateImagePos(GameObject ImageObj, int index)
	{
		...
		{
		case 0:
			ImageObj.transform.localPosition = new Vector2(x / 2f * -1f * 3f, y / 2f * -1f);
			break;
		case 1:
			ImageObj.transform.localPosition = new Vector2(x / 2f * -1f, y / 2f * -1f);
			break;
		case 2:
			ImageObj.transform.localPosition = new Vector2(x / 2f * -1f * 3f, y / 2f * -1f * 3f);
			break;
		case 3:
			ImageObj.transform.localPosition = new Vector2(x / 2f * -1f, y / 2f * -1f * 3f);
			break;
		}
	}
}
