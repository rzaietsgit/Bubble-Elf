...
namespace TMPro
{
	...
	public class InlineGraphicManager : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			{
				UnityEngine.Debug.LogWarning("The InlineGraphics Component must be attached to a TextMesh Pro Object", this);
			}
		...
		private void LoadSpriteAsset(TMP_SpriteAsset spriteAsset)
		{
			if (spriteAsset == null)
			{
				...
				spriteAsset = ((!(tMP_Settings != null) || !(tMP_Settings.spriteAsset != null)) ? (Resources.Load("Sprite Assets/Default Sprite Asset") as TMP_SpriteAsset) : tMP_Settings.spriteAsset);
			}
