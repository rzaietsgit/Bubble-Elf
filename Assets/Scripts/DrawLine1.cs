...
public class DrawLine1 : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Input.GetMouseButton(0) && Time.time - time > 0.01f)
		{
			...
			Vector3 a = UnityEngine.Input.mousePosition + new Vector3(300f, 0f, 0f);
			Vector3 b = Camera.main.WorldToScreenPoint(readyBubbleParent.transform.position);
			direction = a - b;
			direction = direction.normalized;
			direction.z = 0f;
			if (direction.y >= 0.3f && Time.timeScale > 0f && a.y / (float)Screen.height <= 0.95f)
			{
				if (!isReadyAnim)
				{
					isReadyAnim = true;
				...
				Vector3 a2 = new Vector3(direction.x, direction.y, direction.z);
				for (int l = 0; l < pointers.Length; l++)
				{
					...
					if (!flag)
					{
						...
						v = BubbleSpawner.Instance.MoveToPos(vector + a2 / 100f * index);
